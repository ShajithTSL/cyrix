...
def get_bootinfo():
	...
	repair_warehouse_branch_wise = {}
	for branch in branch_list:
		warehouses = frappe.db.get_all("Warehouse List",{"branch":branch.name},["actual_warehouse","repair_warehouse"])


		for b in warehouses:
			actual_warehouse_branch_wise.setdefault(branch.name, []).append(b.actual_warehouse)
			repair_warehouse_branch_wise.setdefault(branch.name, []).append(b.repair_warehouse)
