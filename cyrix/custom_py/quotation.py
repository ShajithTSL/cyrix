...
def create_invoice_request(source,user):
	...
	if doc.quotation_type == "Customer Quotation - Repair" or doc.quotation_type == "Customer Quotation - R - Revised":
		new_doc.request_for = "Job Order"

		new_doc.append("invoice_list",{
			"quotation":doc.name,
		})

		wd = frappe.db.sql(""" select  `tabQuotation Item`.job_order_data from `tabQuotation` 
	...
	if doc.quotation_type == "Customer Quotation - Site Visit" or doc.quotation_type == "Customer Quotation - SV - Revised":
		new_doc.request_for = "Service"
