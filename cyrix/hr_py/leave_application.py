...
class CustomLeaveApplication(LeaveApplication):
	...
	def update_attendance(self):
		...
		holiday_dates = []
		if self.company != "Cyrix TSL - Kuwait":
...
def get_number_of_leave_days(
	...
	bypass_holidays = frappe.db.get_value("Leave Type", leave_type, "bypass_holidays")
	company = frappe.db.get_value("Employee",employee,'company')
	if leave_type == 'Annual Leave' and bypass_holidays:
		if company == "Cyrix TSL - Kuwait":
			number_of_days = flt(number_of_days)
		else:
			number_of_days = flt(number_of_days) - flt(
				get_weekoffs(employee, from_date, to_date, holiday_list=holiday_list)
			)

	elif not frappe.db.get_value("Leave Type", leave_type, "include_holiday"):
		number_of_days = flt(number_of_days) - flt(
...
@frappe.whitelist()
def get_weekoffs(employee, from_date, to_date, holiday_list=None):
	"""get holidays between two dates for the given employee"""
	if not holiday_list:
