...
class CustomPayrollEntry(PayrollEntry):
		...
		for d in employees:
			...
			if leave_salary:
				...
				if ls_start <= start_date.date() and ls_end >= end_date.date():
					frappe.log_error("Skipped Employees from Payroll",d['employee'])
					# Skip fully paid
					continue
				
				# Partially covered — remaining days still need salary
				remaining_days = (end_date.date() - ls_end).days if ls_end < end_date.date() else 0
			else:
			...
			if leaves <= 0 and remaining_days <= 0:
				continue
	...
	def get_salary_components(self, component_type):
		...
		if salary_slips:
			...
			if component_type == "deductions":
				loan_components = frappe.db.sql("""
					SELECT 
						'Loan' AS salary_component,
						sl.total_payment AS amount,
						'deductions' AS parentfield,
						ss.salary_structure,
						ss.employee
					FROM `tabLoan Details` sl
					JOIN `tabSalary Slip` ss ON ss.name = sl.parent
					WHERE ss.name IN %(slips)s
				""", {
					"slips": tuple(d.name for d in salary_slips)
				}, as_dict=True)

			return salary_components + loan_components
			...
			for item in salary_components:
				...
				for cost_center, percentage in employee_cost_centers.items():
					...
					if employee_advance:
						...
						key = (item.salary_component, cost_center, item.employee, frappe.db.get_value("Employee",item.employee, 'branch'))
	...
	def get_account(self, component_dict=None):
		...
		for key, amount in component_dict.items():
			component, cost_center, employee, branch = key
			account = self.get_salary_component_account(component)
			accounting_key = (account, cost_center, employee, branch)
	...
	def get_payable_amount_for_earnings_and_deductions(
		...
		for acc_cc, amount in earnings.items():
			payable_amount = self.get_accounting_entries_and_payable_amount(
				...
				precision,
				party = acc_cc[2],
				branch = frappe.db.get_value("Employee",acc_cc[2],'branch'),
				entry_type="debit",
				accounts=accounts,
			)
	...
	def get_salary_slip_details(self, for_withheld_salaries=False):
		...
		query = (
			...
			.select(
				...
				SalarySlip.total_loan_repayment_,
			)
			.where(
	...
	def process_loan_repayments_for_bank_entry(self, salary_details: list[dict]) -> float:
		...
		total_loan_repayment = sum(flt(slip.get("total_loan_repayment_", 0)) for slip in unique_salary_slips)

		if self.employee_based_payroll_payable_entries:
			for salary_slip in unique_salary_slips:
				if salary_slip.get("total_loan_repayment_"):
					self.set_employee_based_payroll_payable_entries(
						"total_loan_repayment_",
						salary_slip.employee,
						salary_slip.total_loan_repayment_,
		...
		if self.employee_based_payroll_payable_entries:
			for employee, employee_details in self.employee_based_payroll_payable_entries.items():
				je_payment_amount = (
					...
					- (employee_details.get("deductions", 0) or 0)
					- (employee_details.get("total_loan_repayment_", 0) or 0)
				)
...
def get_payroll_period_days(start_date, end_date, employee, company=None):
	...
	if len(payroll_period) > 0:
		...
		if not cint("Company Wise Payroll Days",{"parent":"Payroll Settings","company":company}, "include_holidays"):
...
@frappe.whitelist()
def fill_employee_details_test():
	self = frappe.get_doc("Payroll Entry","HR-PRUN-2026-00001")
	filters = self.make_filters()
	employees = get_employee_list(filters=filters, as_dict=True, ignore_match_conditions=True)
	self.set("employees", [])
	if not employees:
