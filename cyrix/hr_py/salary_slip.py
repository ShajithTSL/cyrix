...
class CustomSalarySlip(SalarySlip):
	...
	def on_submit(self):
		...
		self.create_loan_repayment_from_salary()
	...
	def get_emp_and_working_day_details(self) -> None:
		...
		if self.employee:
			self.set("earnings", [])
			self.set("deductions", [])
			# if hasattr(self, "loans"):              # Commented
				# self.set("loans", [])               # Commented
	...
	def get_working_days_details(self, lwp=None, for_preview=0, lwp_days_corrected=None):
		...
		actual_working_days = date_diff(self.end_date, self.start_date) + 1
		
		if working_days > actual_working_days:
		...
		# working_days = date_diff(self.end_date, self.start_date) + 1    # Commented
		if for_preview:
		...
		count_ = get_leave_days(self.employee,self.start_date, self.end_date)
		annual_count = get_annual_leave_days(self.employee,self.start_date, self.end_date)
		...
		if not cint(include_holidays_in_total_working_days):
			...
			# working_days -= len(holidays)     # Commented
			if working_days < 0:
		...
		def get_overlap_days(employee, company, range_start, range_end, check_start, check_end):
			overlap_start = max(range_start, check_start)
			overlap_end = min(range_end, check_end)
			overlap_days = validate_balance_leaves(company,overlap_start,overlap_end,employee, "Annual Leave")
			return overlap_days if overlap_start <= overlap_end else 0

		if self.start_date and self.end_date and flt(payment_days) > flt(lwp):
			start_date = to_date(self.start_date)
			end_date = to_date(self.end_date)
			count_ = get_leave_days(self.employee, start_date, end_date)
			...
			if leave_salary:
				ls_start = to_date(leave_salary[0]["ls_start"])
				ls_end = to_date(leave_salary[0]["ls_end"])

				if ls_start <= start_date and ls_end >= end_date:
					self.payment_days = 0
				else:
					overlap_days = get_overlap_days(self.employee, self.company, start_date, end_date, ls_start, ls_end)
					self.payment_days = max(flt(payment_days) - flt(lwp) - flt(count_) - flt(overlap_days), 0)
			else:
			...
			if payroll_settings.payroll_based_on == "Attendance":
				if consider_unmarked_attendance_as == "Absent":
					unmarked_days = self.get_unmarked_days(
						include_holidays_in_total_working_days, holidays
					)
	...
	def set_net_pay(self):
		...
		self.net_pay = flt(self.gross_pay) - (
			flt(self.total_deduction) + flt(self.get("total_loan_repayment_"))
		)
	...
	def cancel_loan_repayment_entry(self):
		repayment_names = frappe.get_all(
	...
	def create_loan_repayment_from_salary(self):
		employee = self.employee
		cutoff_date = self.end_date

		loan_deduction = sum(d.total_payment for d in self.loan)
		if loan_deduction <= 0:
			return
		...
		for loan in loans:
			...
			repayment_doc = frappe.new_doc("Loan Request Repayment")
			repayment_doc.against_loan = loan.name
			repayment_doc.posting_date = cutoff_date
			repayment_doc.flags.ignore_permissions = True
			schedule_rows = frappe.get_all(
				...
				fields=["name", "principal_amount", "paid_amount", "total_payment", "payment_date"],
				order_by="payment_date asc"
			)

			# Allocate to past + current rows
			for sch in schedule_rows:
				if remaining_amount <= 0:
					break

				pending = (sch.total_payment or 0) - (sch.paid_amount or 0)
				if pending <= 0:
					continue

				pay_now = min(remaining_amount, pending)
				remaining_amount -= pay_now

				repayment_doc.append("repayment_details", {
					"payment_date": sch.payment_date,
					"reference": sch.name,
					"accrual_type": "Repayment",
					"total_payment": pay_now
				})

			repayment_doc.amount_paid = loan_deduction - remaining_amount
			repayment_doc.salary_slip = self.name
			repayment_doc.save()
			repayment_doc.submit()
	
			
	def validate_repayment_amount(self, loan):
		total_amount_paid, total_payment = frappe.db.get_value(
			"Loan Request",
			{"name": loan},
		...
		remaining = total_payment - total_amount_paid

		# Prevent negative values in case of advance payment
		if remaining < 0:
			remaining = 0
	...
	def calculate_amounts(self,loan):
		...
		schedule_entries = frappe.get_all(
			...
			fields=["name","principal_amount", "total_payment","paid_amount"]
		)
	...
	def set_loan_repayment(self):
		...
		if not self.loan:
			for loan in self.get_loan_details():
				...
				if amounts and amounts["amount"]:
					self.append(
						"loan",
						{
							"loan": loan.name,
							"total_payment": amounts["amount"],
							"principal_amount": amounts["amount"],
							"loan_account": loan.loan_account
						},
					)
		if self.get("loan"):
			for payment in self.get("loan"):
				total_amount = self.validate_repayment_amount(payment.loan)
				...
				self.total_loan_repayment_ += payment.total_payment
		else:
			self.total_loan_repayment_ = 0
	...
	def get_loan_details(self):
		loan_details = frappe.get_all(
			"Loan Request",
			fields=["name", "loan_account", "loan_product", "is_term_loan"],
			filters={
				"employee": self.employee,
				"docstatus": 1,
				"repay_from_salary": 1,
		...
		if allow_tax_exemption:
			for ded in self.deductions:
				if ded.exempted_from_income_tax:
					...
					additional_income -= additional_amount
					# amount_exempted_from_income_tax += flt(amount - additional_amount)     # Commented
	...
	def set_totals(self) -> None:
		...
		if self.salary_slip_based_on_timesheet == 1:
			...
			self.net_pay = (
				flt(self.gross_pay) - flt(self.total_deduction) - flt(self.get("total_loan_repayment_"))
			)
...
def get_leave_days(employee,start_date, end_date):
	count = frappe.db.count("Attendance",{"docstatus":1,"employee":employee,"status": "On Leave","leave_type":"Sick Leave", "attendance_date": ("between", [start_date, end_date])})
	return count

def get_annual_leave_days(employee,start_date, end_date):
	count = frappe.db.count("Attendance",{"docstatus":1,"employee":employee,"status": "On Leave","leave_type":"Annual Leave", "attendance_date": ("between", [start_date, end_date])})
	return count

def get_holiday_dates_for_employee(employee, start_date, end_date):
	"""return a list of holiday dates for the given employee between start_date and end_date"""
	# return only date
	holidays = get_holidays_for_employee_custom(employee, start_date, end_date)

	return [cstr(h.holiday_date) for h in holidays]

def get_holidays_for_employee_custom(employee, start_date, end_date, raise_exception=True, only_non_weekly=False):
	...
	filters = {"parent": holiday_list, "holiday_date": ("between", [start_date, end_date]),"weekly_off":1}
...
def check_for_holidays_between_this_range(leave_start_date,leave_end_date):
	leave_days = get_number_of_leave_days_(
		from_date = leave_start_date, to_date = leave_end_date
	)
	return leave_days

def get_number_of_leave_days_(
	from_date,
	to_date,
) -> float:
