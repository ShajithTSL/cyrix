    ...
    "node_modules/frappe-datatable/dist/frappe-datatable.cjs.js"(exports, module) {
      ...
      var ColumnManager = class {
        ...
        bindResizeColumn() {
          ...
          $2.on(this.header, "mousedown", ".dt-cell .dt-cell__resize-handle", (e, $handle2) => {
            document.body.classList.add("dt-resize");
            const $cell = $handle2.parentNode.parentNode;
            $resizingCell = $cell;
            const {
              colIndex
            } = $2.data($resizingCell);
        ...
        bindPerfectColumnWidth() {
          $2.on(this.header, "dblclick", ".dt-cell .dt-cell__resize-handle", (e, $handle2) => {
            const $cell = $handle2.parentNode.parentNode;
            const { colIndex } = $2.data($cell);
...
<div class="company-branch-container">
	...
	{% 
		...
		let branch = frappe.defaults.get_default("branch");
	%}

	<!-- Company Pill -->
	<button 
		class="pill company-pill {% if (!company) { %}company-alert{% } %}" 
		onclick="company_change()"
		type="button"
	>
		<span class="pill-icon">\u{1F3E2}</span>
	...
	{% if (branch) { %}
	<button 
		class="pill branch-pill"
		onclick="company_change()"
		type="button"
	>
		<span class="pill-icon">\u{1F4CD}</span>
		<span class="pill-text">{%= branch %}</span>
		<span class="pill-arrow">\u25BC</span>
	</button>
	{% } %}
...
.pill {
	...
	/* \u2705 UPDATED */
	max-width: 300px;   /* increase limit */
	min-width: 120px;   /* keeps shape nice */
}

/* Company pill (blue) */
.company-pill {
	background: linear-gradient(135deg, #2f5aa8 0%, #1d3f7a 100%);
}

/* Branch pill (slightly lighter blue) */
.branch-pill {
	background: linear-gradient(135deg, #3a6fd8 0%, #274b9f 100%);
}
...
/* Alert animation */
.company-alert {
	background: linear-gradient(135deg, #ff4d4f 0%, #c1121f 100%);
...
@media (max-width: 768px) {
	...
	/* Option 1: Hide branch completely */
	.branch-pill {
		display: none;
	}

	/* Option 2 (alternative): show branch as tiny */
...
<\/script>`;
  ...
  frappe.views.ListView = class ListView extends frappe.views.ListView {
    refresh() {
      ...
      try {
        ...
        if (!default_company || frappe.ignore_company)
          return;
        if (!frappe.meta.has_field(this.doctype, "company")) {
          return;
        }
        const company_input = (_b = (_a = this.page) == null ? void 0 : _a.fields_dict) == null ? void 0 : _b.company;
        if (company_input) {
  ...
  var SKIP_DOCTYPES = ["User", "Print Format Builder"];
  var $panel;
  var $handle;
  var $anchor;
  var active_frm = null;
  ...
  function setup_activity_panel(frm) {
    if (!frm || !frm.doc || frm.is_new() || frm.doc.__islocal || SKIP_DOCTYPES.includes(frm.doctype)) {
      return hide_handle();
    }
  ...
  function open_panel() {
    if (!active_frm || !active_frm.footer)
      return;
    ...
    setTimeout(() => hide_view_logs($panel), 600);
  }
